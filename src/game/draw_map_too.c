...
/*   Updated: 2025/07/09 21:40:55 by oissa            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	line->x0 = main->map_2d.offsetX + x * TILE_SIZE;
	line->y0 = main->map_2d.offsetY + y * TILE_SIZE;
	line->x1 = main->map_2d.offsetX + (x + 1) * TILE_SIZE;
	line->y1 = main->map_2d.offsetY + y * TILE_SIZE;
...
void	draw_vertical_line(t_main *main, int x, int y, t_line *line)
{
	line->x0 = main->map_2d.offsetX + x * TILE_SIZE;
	line->y0 = main->map_2d.offsetY + y * TILE_SIZE;
	line->x1 = main->map_2d.offsetX + x * TILE_SIZE;
	line->y1 = main->map_2d.offsetY + (y + 1) * TILE_SIZE;
	line->color = main->map_2d.border;
	line->thickness = 1;
	mlx_draw_line_thick(main->game.image, *line);
}
...
void	draw_player_rectangle(t_main *t)
{
	...
	t->map_2d.px = t->map_2d.offsetX + (t->player.x - (int)t->player.x)
		* TILE_SIZE + t->map_2d.map_size / 2 * TILE_SIZE;
	t->map_2d.py = t->map_2d.offsetY + (t->player.y - (int)t->player.y)
		* TILE_SIZE + t->map_2d.map_size / 2 * TILE_SIZE;
